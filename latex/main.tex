...
for key := range tf {
    ...
    \begin{table}[H]
\centering
\caption{Tempo de inferência do modelo Sentence-BERT para diferentes tamanhos de consulta}
\label{tab:bert_inferencia}
\begin{tabular}{lcc}
\hline
\textbf{Configuração} & \textbf{Tempo Total} & \textbf{Tempo Médio} \\
\hline
Documentos (1.648) & 1m27,31s & 52,98 ms/doc \\
Consulta -- 10 palavras & 2,53 s & 49,68 ms/frase \\
Consulta -- 20 palavras & 2,49 s & 49,95 ms/frase \\
Consulta -- 40 palavras & 2,50 s & 50,14 ms/frase \\
\hline
\end{tabular}
\end{table}


Apesar da aparente estabilidade no tempo médio de inferência do modelo Sentence-BERT
independentemente do tamanho da consulta, os dados apresentados não permitem conclusões
definitivas sobre a escalabilidade semântica do modelo. O conjunto de experimentos foi
executado sob um número limitado de prompts, em um ambiente de hardware específico e
com um único modelo de embedding. Dessa forma, os resultados devem ser interpretados
como indicativos do comportamento observado neste cenário controlado, não sendo
suficientes para generalizações amplas ou afirmações conclusivas sobre desempenho em
