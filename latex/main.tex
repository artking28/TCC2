...
%    Esta pesquisa oferece uma análise quantitativa desse \textit{trade-off}. Ao comparar diretamente o desempenho, o tempo de resposta e o uso de recursos de diferentes abordagens — uma prática já adotada em trabalhos que avaliam modelos como BERT e ChatGPT por meio de métricas de acurácia, precisão e F1-score \cite{citacao66} — pretendemos fornecer dados concretos que auxiliem na tomada de decisão sobre qual tecnologia é mais adequada para diferentes necessidades do ecossistema jurídico, movendo o debate do campo teórico para uma avaliação de aplicabilidade prática.
    ...
    \begin{figure}[H]
        ...
        \includegraphics[width=1.0\textwidth]{assets/lem_stem.png}
...
% (Esta seção foi mantida como estava, pois já era muito boa)
    ...
    O trabalho influente de Robertson, \cite{robertson1995okapi} apresenta os resultados do sistema Okapi na TREC-3 e acaba servindo como uma das principais evidências empíricas da superioridade do BM25 sobre o TF-IDF tradicional. Os autores não buscavam criar um sistema novo, mas sim avaliar e aprimorar um modelo probabilístico de recuperação em um ambiente de testes rigoroso.
...
for key := range tf {
    ...
    Este trabalho avaliou, de forma prática, diferentes técnicas de recuperação de informação, comparando algoritmos estatísticos implementados em Go com um modelo vetorial baseado em \textit{transformers}. A análise, conduzida sobre um \textit{corpus} jurídico brasileiro extenso, buscou entender até que ponto é possível equilibrar custo computacional e precisão semântica em sistemas de busca especializados.


    \section{Síntese dos Resultados e Contribuições}

    Os experimentos — 72 cenários medidos pelo Coeficiente de Spearman — confirmaram hipóteses levantadas na revisão teórica e apontaram para um “ponto de equilíbrio” claro entre desempenho e qualidade no domínio jurídico.

    \subsection{Velocidade: BM25 Superando o TF-IDF}

    Um resultado marcante foi o desempenho superior do BM25 em tempo de execução, especialmente nos cenários com Bigramas e Trigramas. Embora o TF-IDF seja matematicamente mais simples, o BM25 se mostrou até \textbf{dez vezes mais rápido} em alguns testes.

    Isso ocorreu por causa da otimização adotada na implementação (Seção 3.4), que evitou cálculos desnecessários ao descartar termos com frequência de documento igual a zero. Em N-gramas maiores, onde a esparsidade cresce, esse “atalho” reduz de forma significativa o processamento. Esse achado reforça que eficiência não depende apenas da fórmula, mas de como o algoritmo lida com o volume real de dados.

    \subsection{O Custo da Indexação Contextual}

    O consumo de memória aumentou de forma acentuada conforme o tamanho dos N-gramas cresceu:

    \begin{itemize}
        \item \textbf{Unigramas:} pico de 143 MB.
        \item \textbf{Bigramas:} cerca de 2.33 GB.
        \item \textbf{Trigramas:} quase 5 GB.
    \end{itemize}

    O salto entre unigramas e trigramas (aprox. 32 vezes) evidencia o custo elevado para manter índices mais ricos. Embora modelos de \textit{embeddings} ofereçam maior precisão, o investimento em \textit{hardware} torna-se um fator crítico. Assim, para sistemas de busca com restrições orçamentárias, unigramas ou bigramas acabam sendo as opções mais viáveis.
