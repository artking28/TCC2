...
% ========================================================================
    ...
    \begin{enumerate}
        \item Construir um \textit{corpus} textual robusto a partir da coleta automatizada via \textit{scraping} e da higienização de documentos legislativos, com foco em Propostas de Lei e Propostas de Emenda à Constituição obtidas do portal da Câmara dos Deputados.
...
% (Esta seção foi mantida como estava, pois já era muito boa)
    ...
    O corpus reúne cerca de 5.000 páginas de documentos legislativos (Projetos de Lei e Propostas de Emenda à Constituição), obtidos via \emph{web scraping} do Portal da Câmara dos Deputados. A extração de texto dos PDFs para texto simples foi feita de forma concorrente em Go.

    \section{Parametrização do Motor de Busca e Hardware}

    \subsection{Algoritmos de Ranqueamento e Indexação}

    Os modelos TF-IDF e Okapi BM25 foram implementados em Go.

    A indexação testou diferentes granularidades: unigramas, bigramas, trigramas e variações de skip-grams com até dois saltos. O índice invertido foi armazenado em SQLite.

    Para o BM25, foram usados os hiperparâmetros fixos: $\mathbf{k_1 = 1.5}$ e $\mathbf{b = 0.75}$.

    \subsection{Queries e Modelo de Gabarito}

    As \textit{queries} de teste foram geradas pelos modelos ChatGPT e Grok.

    O gabarito semântico usado na avaliação foi o Sentence-BERT (\texttt{paraphrase-multilingual-MiniLM-L12-v2}), aplicando Similaridade de Cosseno para medir relevância.

    \subsection{Hardware}

    Os testes foram executados no seguinte hardware:

    \begin{itemize}

        \item \textbf{Processador:} Apple M1 (2020)

        \item \textbf{RAM:} 16 GB a 4266 MHz

    \end{itemize}

    \subsection{Métricas de Avaliação}

    A eficácia do ranqueamento foi medida pelo coeficiente de Spearman ($\rho$), comparando o ranqueamento estatístico com o semântico.

    A eficiência computacional foi avaliada pelo tempo de execução (latência de busca) e pico de uso de memória em cada cenário.
...
% --- CAPÍTULO 4: DESENVOLVIMENTO ---
    ...
    O fluxo de coleta é iniciado pela definição do escopo, etapa na qual o algoritmo varre as páginas de resultados do portal para identificar os documentos disponíveis com base nas categorias legislativas de interesse. A execução ocorre de forma paralela, disparando múltiplas requisições simultâneas para maximizar o uso da banda. No entanto, para mitigar riscos de bloqueio de IP por excesso de requisições (\textit{throttling}) ou a interpretação do tráfego como um ataque de Negação de Serviço (DoS), implementou-se um mecanismo de \textit{rate limiting} com um intervalo de segurança fixo de 500 milissegundos entre as chamadas, garantindo a estabilidade e o respeito às políticas do portal governamental.

    Na etapa de processamento do conteúdo, a biblioteca \texttt{goquery} analisa o HTML de cada página para filtrar e extrair apenas as URLs que apontam para o inteiro teor dos documentos em formato \texttt{.pdf}. Concluída a identificação, os arquivos são baixados via \textit{stream} e salvos diretamente no diretório local \texttt{./misc/corpus/pdf}. Este processo de persistência mantém a nomenclatura original dos arquivos, um requisito essencial para garantir a futura rastreabilidade dos dados no \textit{corpus}.
